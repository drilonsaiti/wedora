...
export async function createWedding(input: CreateWeddingInput) {
    ...
    if (slugTaken)
        return {
            success: false as const,
            error: "Kjo URL është e zënë, provoni një tjetër",
        };
...
export async function updateWedding(
    ...
    if (input.bride_email !== undefined)
        updates.bride_email = input.bride_email || null;
    ...
    if (Object.keys(updates).length > 0) {
        ...
        if (error)
            return {
                success: false as const,
                error: "Dështoi ruajtja e ndryshimeve",
            };
    }
    ...
    if (
        ...
        if (input.theme_hue !== undefined)
            settingsUpdates.theme_hue = input.theme_hue;
        if (input.enable_find_seat !== undefined)
            settingsUpdates.enable_find_seat = input.enable_find_seat;
        if (input.enable_photo_upload !== undefined)
            settingsUpdates.enable_photo_upload = input.enable_photo_upload;
        if (input.auto_approve_uploads !== undefined)
            settingsUpdates.auto_approve_uploads = input.auto_approve_uploads;
        if (input.max_photos_total !== undefined)
        ...
        if (error)
            return { success: false as const, error: "Dështoi ruajtja e cilësimeve" };
    }
    ...
    const emailChecks: {
        ...
        {
            email: input.groom_email,
            oldEmail: currentWedding.groom_email,
            role: "groom",
        },
        {
            email: input.bride_email,
            oldEmail: currentWedding.bride_email,
            role: "bride",
        },
    ];
