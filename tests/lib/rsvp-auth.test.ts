...
function mockService(options: Parameters<typeof makeMockService>[0]) {
    vi.mocked(createServiceClient).mockReturnValue(
        makeMockService(options) as unknown as ReturnType<
...
// rsvpApiAccess is add-on-only (see lib/plans.ts) -- entitled here by
// default so these tests exercise the key/rate-limit logic they're
// actually about, not the plan gate. Tests that specifically cover the
// plan gate override this.
const WEDDING = {
    id: "wedding-1",
    slug: "sara-drilon",
    plan: "basic",
    addons: ["rsvpApiAccess"],
};
...
describe("resolveRsvpApiActor", () => {
    ...
    it("rejects a valid, non-revoked key with 403 when the wedding's plan no longer includes RSVP API access", async () => {
        const { raw, hash } = generateRsvpApiKey();

        const downgradedWedding = {
            ...WEDDING,
            plan: "basic",
            addons: [], // rsvpApiAccess add-on removed
        };

        mockService({
            wedding: { data: downgradedWedding, error: null },
            apiKey: {
                data: {
                    id: "key-1",
                    wedding_id: WEDDING.id,
