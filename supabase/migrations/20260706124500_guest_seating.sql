...
create
policy "Public can read tables"
  on public.tables for
select
    using (true);

-- Admins can manage tables
create
policy "Admins can manage tables"
...
create
policy "Public can read guests"
  on public.guests for
select
    using (true);

-- Admins can manage guests
create
policy "Admins can manage guests"
  on public.guests for all
  using (public.is_admin());

-- Function to generate initials
create
or replace function public.generate_initials(first_name text, last_name text)
returns text
language plpgsql
as $$
begin
...
create
or replace function public.handle_guest_initials()
returns trigger
language plpgsql
as $$
begin
  new.initials
:= public.generate_initials(new.first_name, new.last_name);
return new;
end;
$$;
