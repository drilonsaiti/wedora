...
create table if not exists public.photo_upload_rate_limits (
    ...
    primary key (wedding_id, scope, key_hash)
);
...
create or replace function public.consume_photo_upload_rate_bucket(
    p_wedding_id uuid,
    p_scope text,
    p_key_hash text,
    p_limit integer,
...
declare
    ...
    v_retry integer;
begin
    if p_scope not in ('session', 'ip') then
...
create or replace function public.begin_guest_photo_upload(
    p_event_id uuid,
    p_session_id text,
...
declare
    v_wedding_id uuid;
    v_enable_photo_upload boolean;
    v_max_photos_total integer;
    v_max_photos_per_guest integer;
    v_total_count bigint;
    v_session_count bigint;
    v_retry integer;
begin
    if p_session_id is null or length(trim(p_session_id)) < 8 then
        return query select false, null::uuid, 'INVALID_FILE'::text, null::integer;
        return;
    end if;
    ...
    if not found or v_wedding_id is null then
        return query select false, null::uuid, 'INVALID_EVENT'::text, null::integer;
        return;
    end if;

    if coalesce(v_enable_photo_upload, false) = false then
        return query select false, v_wedding_id, 'UPLOAD_DISABLED'::text, null::integer;
        return;
    end if;

    -- Keep the limiter table bounded without needing a separate cron.
    -- Only old buckets for this wedding are touched.
    delete from public.photo_upload_rate_limits
    ...
    if v_max_photos_total is not null then
        select count(*)
        into v_total_count
        from public.photos
        where wedding_id = v_wedding_id;

        if v_total_count >= v_max_photos_total then
            return query select false, v_wedding_id, 'WEDDING_LIMIT_REACHED'::text, null::integer;
            return;
        end if;
    end if;

    if v_max_photos_per_guest is not null then
        select count(*)
        ...
        if v_session_count >= v_max_photos_per_guest then
            return query select false, v_wedding_id, 'PHOTO_LIMIT_REACHED'::text, null::integer;
            return;
        end if;
    end if;
    ...
    if v_retry > 0 then
        return query select false, v_wedding_id, 'RATE_LIMITED'::text, v_retry;
        return;
    end if;

    if p_ip_key_hash is not null then
        v_retry := public.consume_photo_upload_rate_bucket(
            v_wedding_id,
            'ip',
            p_ip_key_hash,
...
create or replace function public.finalize_guest_photo_upload(
    p_photo_id uuid,
    p_event_id uuid,
    p_wedding_id uuid,
    p_session_id text,
...
begin
    ...
    if not found or v_event_wedding_id is distinct from p_wedding_id then
    ...
    if not found or coalesce(v_enable_photo_upload, false) = false then
        return query select false, 'UPLOAD_DISABLED'::text;
        return;
    end if;
    ...
    if v_max_photos_total is not null then
        ...
        if v_total_count >= v_max_photos_total then
            return query select false, 'WEDDING_LIMIT_REACHED'::text;
            return;
        end if;
    end if;
    ...
    if v_max_photos_per_guest is not null then
        ...
        if v_session_count >= v_max_photos_per_guest then
            return query select false, 'PHOTO_LIMIT_REACHED'::text;
            return;
        end if;
    end if;
...
create or replace function public.register_photo_upload_cleanup_job(
    p_photo_id uuid,
    p_original_path text,
    p_thumbnail_path text
)
...
as $$
begin
    insert into public.photo_upload_cleanup_jobs (
        photo_id,
        original_path,
        thumbnail_path
    ) values (
...
create or replace function public.complete_photo_upload_cleanup_job(
    p_photo_id uuid
)
returns void
language plpgsql
...
as $$
begin
    delete from public.photo_upload_cleanup_jobs
    where photo_id = p_photo_id;
end;
$$;

create or replace function public.get_stale_photo_upload_cleanup_jobs(
...
begin
    ...
    delete from public.photo_upload_cleanup_jobs j
    where exists (
        select 1
...
create or replace function public.mark_photo_upload_cleanup_failed(
    p_photo_id uuid,
    p_error text
)
returns void
...
begin
    ...
    set
        attempts = attempts + 1,
        last_error = left(coalesce(p_error, 'Unknown cleanup error'), 1000),
...
grant execute on function public.begin_guest_photo_upload(uuid, text, text, text, integer, integer, integer) to service_role;
grant execute on function public.finalize_guest_photo_upload(uuid, uuid, uuid, text, text, text, text, text, bigint, integer, integer, boolean) to service_role;
grant execute on function public.register_photo_upload_cleanup_job(uuid, text, text) to service_role;
grant execute on function public.complete_photo_upload_cleanup_job(uuid) to service_role;
grant execute on function public.get_stale_photo_upload_cleanup_jobs(integer, integer) to service_role;
grant execute on function public.mark_photo_upload_cleanup_failed(uuid, text) to service_role;
