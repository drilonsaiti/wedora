...
alter table public.guests
    add column if not exists rsvp_updated_by_key_id uuid
        references public.wedding_rsvp_api_keys(id) on delete set null;
...
create table if not exists public.rsvp_api_rate_limits (
    ...
    primary key (wedding_id, key_hash)
);
...
create or replace function public.consume_rsvp_rate_bucket(
    p_wedding_id uuid,
    p_key_hash text,
    p_limit integer,
...
begin
    ...
    delete from public.rsvp_api_rate_limits
    ...
    if v_now >= v_window_started_at + make_interval(secs => p_window_seconds) then
        ...
        where wedding_id = p_wedding_id and key_hash = p_key_hash;
...
grant execute on function public.consume_rsvp_rate_bucket(uuid, text, integer, integer)
    to service_role;
