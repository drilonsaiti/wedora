...
ALTER TABLE public.photos
    ADD COLUMN wedding_id uuid REFERENCES public.weddings (id) ON DELETE CASCADE;
ALTER TABLE public.gallery_tokens
    ADD COLUMN wedding_id uuid REFERENCES public.weddings (id) ON DELETE CASCADE;
...
SET wedding_id = v_wedding_id;
UPDATE public.venue_elements
SET wedding_id = v_wedding_id;
UPDATE public.photos
SET wedding_id = v_wedding_id;
UPDATE public.gallery_tokens
SET wedding_id = v_wedding_id;
...
CREATE
OR REPLACE FUNCTION public.is_wedding_owner(target_wedding_id uuid)
RETURNS boolean
LANGUAGE sql
SECURITY DEFINER
STABLE
...
CREATE
OR REPLACE FUNCTION public.is_platform_admin()
RETURNS boolean
LANGUAGE sql
SECURITY DEFINER
STABLE
...
CREATE
POLICY "admins_manage_own_wedding" ON public.weddings
...
CREATE
POLICY "admins_manage_own_wedding_settings" ON public.wedding_settings
  FOR ALL TO authenticated
...
DROP
POLICY IF EXISTS "Admins can manage tables" ON public.tables;
CREATE
POLICY "admin_owns_wedding_tables" ON public.tables
  FOR ALL TO authenticated
  USING (public.is_wedding_owner(wedding_id) OR public.is_platform_admin())
  WITH CHECK (public.is_wedding_owner(wedding_id) OR public.is_platform_admin());

DROP
POLICY IF EXISTS "Admins can manage guests" ON public.guests;
CREATE
POLICY "admin_owns_wedding_guests" ON public.guests
  FOR ALL TO authenticated
  USING (public.is_wedding_owner(wedding_id) OR public.is_platform_admin())
  WITH CHECK (public.is_wedding_owner(wedding_id) OR public.is_platform_admin());

DROP
POLICY IF EXISTS "Admins can manage venue elements" ON public.venue_elements;
CREATE
POLICY "admin_owns_wedding_venue_elements" ON public.venue_elements
  FOR ALL TO authenticated
  USING (public.is_wedding_owner(wedding_id) OR public.is_platform_admin())
  WITH CHECK (public.is_wedding_owner(wedding_id) OR public.is_platform_admin());

DROP
POLICY IF EXISTS "Admins can manage events" ON public.events;
CREATE
POLICY "admin_owns_wedding_events" ON public.events
  FOR ALL TO authenticated
  USING (public.is_wedding_owner(wedding_id) OR public.is_platform_admin())
  WITH CHECK (public.is_wedding_owner(wedding_id) OR public.is_platform_admin());

DROP
POLICY IF EXISTS "Admins can read all photos" ON public.photos;
DROP
POLICY IF EXISTS "Admins can update photos" ON public.photos;
DROP
POLICY IF EXISTS "Admins can delete photos" ON public.photos;
CREATE
POLICY "admin_owns_wedding_photos" ON public.photos
  FOR ALL TO authenticated
  USING (public.is_wedding_owner(wedding_id) OR public.is_platform_admin())
  WITH CHECK (public.is_wedding_owner(wedding_id) OR public.is_platform_admin());

DROP
POLICY IF EXISTS "Admins can manage gallery tokens" ON public.gallery_tokens;
CREATE
POLICY "admin_owns_wedding_gallery_tokens" ON public.gallery_tokens
  FOR ALL TO authenticated
  USING (public.is_wedding_owner(wedding_id) OR public.is_platform_admin())
  WITH CHECK (public.is_wedding_owner(wedding_id) OR public.is_platform_admin());
...
CREATE
OR REPLACE FUNCTION public.is_couple_for_wedding(target_wedding_id uuid)
RETURNS boolean
LANGUAGE sql
SECURITY DEFINER
STABLE
...
AS $$
SELECT (auth.jwt() - > 'app_metadata' ->> 'wedding_id') ::uuid = target_wedding_id
    AND (auth.jwt() -> 'app_metadata' ->> 'role') = 'couple';
$$;

DROP
POLICY IF EXISTS "couple_read_guests" ON public.guests;
CREATE
POLICY "couple_read_guests" ON public.guests
  FOR
...
DROP
POLICY IF EXISTS "couple_read_tables" ON public.tables;
CREATE
POLICY "couple_read_tables" ON public.tables
  FOR
SELECT TO authenticated
    USING (
    is_couple_for_wedding(wedding_id)
...
DROP
POLICY IF EXISTS "couple_read_photos" ON public.photos;
CREATE
POLICY "couple_read_photos" ON public.photos
  FOR
SELECT TO authenticated
    USING (
    is_couple_for_wedding(wedding_id)
...
DROP
POLICY IF EXISTS "couple_delete_photos" ON public.photos;
CREATE
POLICY "couple_delete_photos" ON public.photos
  FOR DELETE
TO authenticated
  USING (
    is_couple_for_wedding(wedding_id)
...
DROP
POLICY IF EXISTS "couple_read_wedding_settings" ON public.wedding_settings;
CREATE
POLICY "couple_read_wedding_settings" ON public.wedding_settings
  FOR
SELECT TO authenticated
