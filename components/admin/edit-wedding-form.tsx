...
export function EditWeddingForm({
    ...
    const t = useTranslations("weddings.settingsPage");
    ...
    try {
        ...
        if (match) {
            initialHue = Number.parseInt(match[1], 10);
        }
    } catch {
        initialHue = 355;
    }
    ...
    const onSubmit = async (data: EditWeddingInput) => {
        ...
        try {
            ...
            if (!result.success) {
                setServerError(result.error ?? t("saveFailed"));
            ...
            const failed = result.failedEmails ?? [];
             ...
             */
            const needsAttention = credentials.length > 0 || failed.length > 0;

            if (onSuccess && !needsAttention) {
                onSuccess();
            }
        } catch (error) {
    ...
    return (
        <>
            ...
            {/* =====================================
                HEADER
            ===================================== */}
            <header className={cn(modal ? "mb-7 pr-10" : "mb-8")}>
                ...
                <h1
                    className={cn(
                        ...
                        modal ? "text-3xl sm:text-4xl" : "text-4xl sm:text-5xl",
                    )}
                >
                    {t("title")}
                </h1>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
                {/* =====================================
                    COUPLE
                ===================================== */}
                <SettingsSection icon={Users} title={t("coupleDetails")} modal={modal}>
                    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                        <FormField
                            label={tw("groomName")}
                            error={translateValidation(errors.groom_name?.message, tv)}
                        >
                            <input
                                {...register("groom_name")}
                                type="text"
                        ...
                        <FormField
                            label={tw("brideName")}
                            error={translateValidation(errors.bride_name?.message, tv)}
                        >
                            <input
                                {...register("bride_name")}
                                type="text"
                ...
                {/* =====================================
                    ACCOUNTS
                ===================================== */}
                <SettingsSection icon={Mail} title={t("accounts")} modal={modal}>
                    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                        <FormField
                            label={t("groomEmail")}
                            error={translateValidation(errors.groom_email?.message, tv)}
                        >
                            <input
                        ...
                        <FormField
                            ...
                            error={translateValidation(errors.bride_email?.message, tv)}
                        >
                            <input
                ...
                >
                    <FormField
                        label={tw("date")}
                        error={translateValidation(errors.wedding_date?.message, tv)}
                    >
                        <input
                    ...
                    {wedding.slug && (
                        <div className="mt-4">
                            <p className="label-wedding">{t("publicUrl")}</p>
                ...
                {/* =====================================
                    THEME
                ===================================== */}
                <SettingsSection icon={Palette} title={t("theme")} modal={modal}>
                    <ColorPicker
                        value={themeHue}
                        onChange={(hue) => {
                            setValue("theme_hue", hue, {
                                shouldDirty: true,
                            });
                        }}
                    />
                ...
                {/* =====================================
                    FEATURES
                ===================================== */}
                <SettingsSection icon={MapPin} title={t("features")} modal={modal}>
                    <div className="space-y-2">
                        ...
                        <FeatureToggle
                            icon={ShieldCheck}
                            title={t("autoApproveUploads")}
                            description={t("autoApproveUploadsDescription")}
                        >
                            <input
                                type="checkbox"
                                {...register("auto_approve_uploads")}
                                className="h-4 w-4 shrink-0 accent-[hsl(var(--primary))]"
                            />
                        </FeatureToggle>
                    </div>
                    ...
                    {errors.enable_find_seat?.message && (
                        <p className="mt-2 text-xs text-destructive">
                            {translateValidation(errors.enable_find_seat.message, tv)}
                        </p>
                    )}
                </SettingsSection>
                    ...
                    <section className="rounded-[1.5rem] border border-[hsl(var(--primary))]/15 bg-[hsl(var(--accent))]/45 p-5">
                        <div className="mb-5">
                            ...
                            <p className="mt-1 text-xs leading-5 text-muted-foreground">
                                {t("newCredentialsDescription")}
                            </p>
                        </div>

                        <div className="overflow-hidden rounded-2xl border border-border/60 bg-card">
                            <div className="divide-y divide-border/60">
                                {newCredentials.map((credential, index) => (
                ...
                {/* =====================================
                    SAVE
                ===================================== */}
                <div className={cn("z-20 pt-2", !modal && "sticky bottom-4")}>
                    <div
                        className={cn(
                            modal
                                ? ""
                    ...
                    >
                        <button
                            ...
                            className={cn(
                                ...
                                saved && "bg-foreground text-background",
                            )}
                        >
                            {isSubmitting ? (
                                <>
    ...
    return (
        <section
            className={cn(
                ...
                modal ? "rounded-[1.5rem] p-5" : "rounded-[2rem] p-5 sm:p-6",
            )}
        >
...
function translateValidation(
    ...
    return translate(message.replace("validation.", ""));
}
