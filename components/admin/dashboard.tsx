...
export function AdminDashboard({
  ...
  const goPrev = async () => { if (modalIndex > 0) await openModal(photos[modalIndex - 1]) }
  const goNext = async () => { if (modalIndex < photos.length - 1) await openModal(photos[modalIndex + 1]) }
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="relative group">
              ...
              {/* Dropdown */}
              <div className="absolute right-0 top-full mt-1 w-44 bg-card border border-border rounded-xl shadow-lg overflow-hidden opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-150 z-10">
                <button
        ...
        >
          ...
          >
            <div className="flex items-center justify-between mb-5">
              <div>
                <h2 className="font-serif text-2xl font-light text-[hsl(var(--dark))]">Share Gallery</h2>
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-2 right-2 flex gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
        <QuickActionBtn
          onClick={(e) => { e.stopPropagation(); onUpdate(photo.id, { favourite: !photo.favourite }) }}
        ...
        <QuickActionBtn
          onClick={(e) => { e.stopPropagation(); onUpdate(photo.id, { hidden: !photo.hidden }) }}
          loading={isActionLoading}
        >
      ...
      {photo.guest_name && (
        <div className="absolute bottom-2 left-2 bg-black/60 text-white text-xs px-2 py-0.5 rounded-full font-sans truncate max-w-[80%]">
          {photo.guest_name}
        </div>
      )}
    </div>
